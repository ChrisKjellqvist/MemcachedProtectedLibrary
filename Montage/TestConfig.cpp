...
void GlobalTestConfig::parseCommandLine(int argc, char** argv){
	...
	// Read command line
	while ((c = getopt (argc, argv, "s:d:w:o:i:t:m:a:vhz")) != -1){
		switch (c) {
			case 's':
...
void GlobalTestConfig::buildDFSAffinity_helper(hwloc_obj_t obj){
	...
	if((int)affinities.size()>=task_num){return;}
...
void GlobalTestConfig::buildAffinity(){
	...
	if((int)affinities.size()<task_num){
...
GlobalTestConfig::GlobalTestConfig():tests(),testNames(),outFile(){}
